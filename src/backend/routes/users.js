...
// --- THIS IS THE FIX ---
// The specific '/me' route is now placed BEFORE the dynamic '/:firebase_uid' route.
...
// UPDATE a user's role by firebase_uid
router.put('/:firebase_uid', authenticate, async (req, res) => {
  ...
  try {
    const result = await query('UPDATE users SET role_id = $1 WHERE firebase_uid = $2 RETURNING *', [role_id, firebase_uid]);
    if (result.rows.length === 0) return res.status(404).json({ message: 'User not found' });
    await logAudit({
      actorUserId,
...
router.delete('/:firebase_uid', authenticate, async (req, res) => {
  // You would add role checks here to ensure only an admin can perform this
  const { firebase_uid } = req.params;
  const actorUserId = req.user?.user_id;
  try {
